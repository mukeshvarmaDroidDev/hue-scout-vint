    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-end overflow-hidden">
        ...
        <motion.div 
          ...
          className="relative z-10 w-full max-w-2xl h-full bg-brand-beige flex flex-col justify-between shadow-2xl overflow-y-auto"
        >
          {/* Close button */}
          <button 
            onClick={onClose}
          ...
          <div className="flex-1 p-6 md:p-12 space-y-10">
            {/* Gallery Section */}
            <div className="relative aspect-[4/5] w-full bg-brand-cream overflow-hidden border border-brand-concrete/30">
              <img 
              ...
              {item.images.length > 1 && (
                <>
                  <button 
                    ...
                    className="absolute left-4 top-1/2 -translate-y-1/2 p-2 bg-brand-beige/80 border border-brand-concrete/30 rounded-full text-brand-charcoal hover:bg-brand-beige cursor-pointer"
                  >
                    <ChevronLeft size={16} />
                  </button>
                  <button 
                    onClick={handleNextImage} 
                    className="absolute right-4 top-1/2 -translate-y-1/2 p-2 bg-brand-beige/80 border border-brand-concrete/30 rounded-full text-brand-charcoal hover:bg-brand-beige cursor-pointer"
                  >
            ...
            <div className="space-y-6">
              ...
              {/* Fabric Specs Grid */}
              <div className="grid grid-cols-2 gap-6 border-y border-brand-charcoal/10 py-6 text-xs tracking-wider">
                <div>
          ...
          {/* Action Footer */}
          <div className="p-6 md:p-12 border-t border-brand-charcoal/10 bg-brand-cream/40 flex flex-col gap-4">
            <button 
